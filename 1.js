...
const getNumbersOnly = (s) => s.replace(/[a-z]/ig, '')
const getFirstLastDigits = n => (n[0] + n[n.length-1])*1
const getAnswerA = (input) => {
    const cleanedInput = cleanInput(input)
    return cleanedInput.reduce((accumulator, currentValue) => {
        const numbersOnly = getNumbersOnly(currentValue)
        const firstAndLastDigits = getFirstLastDigits(numbersOnly)
        return accumulator + firstAndLastDigits
    }, 0)
}

const answerExampleA = getAnswerA(exampleInputA)
console.log(answerExampleA)
const answerA = getAnswerA(inputA)
console.log(answerA)

const numis = { one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8, nine: 9 }

const replaceNumis = (s) => {
    let newString = s
    for (let [key, value] of Object.entries(numis)) {
        while (newString.includes(key)) newString = newString.replace(key, key[0]+value+key[key.length-1])
    }
    return newString
}
...
const getAnswerB = (input) => {
    const cleanedInput = cleanInput(input)
    return cleanedInput.reduce((accumulator, currentValue) => {
        const replacedNumis = replaceNumis(currentValue)
        const numbersOnly = getNumbersOnly(replacedNumis)
        const firstAndLastDigits = getFirstLastDigits(numbersOnly)
        return accumulator + firstAndLastDigits
    }, 0)
}

const answerExampleB = getAnswerB(exampleInputB)
console.log(answerExampleB)
const answerB = getAnswerB(inputA)
console.log(answerB)
