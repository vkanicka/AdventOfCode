...
`
const conditionA = {red: 12, green: 13, blue: 14}
const cleanInput = (s) => s.split('\n').filter(x => x != '')

const getAnswerA = (input) => {
    const cleanedInput = cleanInput(input)
    return cleanedInput.reduce((accumulator, currentValue) => {
        let isGamePossible = true
        let [gameId, allHands] = currentValue.split(': ')
        gameId = gameId.replace('Game ','')*1
        allHands = allHands.split('; ').join(', ').split(', ').map(x => x.split(' '))
        for (let [count, color] of allHands) {
            if (count > conditionA[color]) {
                isGamePossible = false
            }
        }
        return isGamePossible ? accumulator + gameId : accumulator
    }, 0)
}
console.log(getAnswerA(exampleInputA))
console.log(getAnswerA(inputA))

const getAnswerB = (input) => {
    const cleanedInput = cleanInput(input)
    return cleanedInput.reduce((accumulator, currentValue) => {
        let [gameId, allHands] = currentValue.split(': ')
        gameId = gameId.replace('Game ','')*1
        allHands = allHands.split('; ').join(', ').split(', ').map(x => x.split(' '))
        let minGame = {red: 0, green: 0, blue: 0}
        for (let [count, color] of allHands) {
            count*=1
            if (count > minGame[color]) {
                minGame[color] = count
            }
        }
        let gameCube = minGame.red*minGame.green*minGame.blue
        return accumulator + gameCube
    }, 0)
}
console.log(getAnswerB(exampleInputA))
console.log(getAnswerB(inputA))